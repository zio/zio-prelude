...
package object prelude
  ...
  type Covariant[F[+_]]           = TriCovariant[({ type lambda[R, E, +A] = F[A] })#lambda]
  ...
  type RightCovariant[<=>[_, +_]] = TriCovariant[({ type lambda[R, E, +A] = E <=> A })#lambda]
  ...
  object classic {
    ...
    type Trifunctor[T[+_, +_, +_]] = TriFullCovariant[T]
  }
