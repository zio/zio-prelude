    ...
    def apply(regex: Assertion.Regex)(using Quotes): Expr[Assertion.Regex] =
      regex match {
        ...
        case Assertion.Regex.CharacterSet(set, reversed) =>
          '{ Assertion.Regex.CharacterSet(${ Expr(set) }, ${ Expr(reversed) }) }
        case Assertion.Regex.Range(start, end, reversed) =>
          '{ Assertion.Regex.Range(${ Expr(start) }, ${ Expr(end) }, ${ Expr(reversed) }) }
        case Assertion.Regex.Start                       => '{ Assertion.Regex.Start }
        case Assertion.Regex.Repeat(regex, min, max)     =>
          '{ Assertion.Regex.Repeat(${ Expr(regex) }, ${ Expr(min) }, ${ Expr(max) }) }
  ...
  given FromExpr[Assertion.Regex] with {
    def unapply(assertion: Expr[Assertion.Regex])(using Quotes): Option[Assertion.Regex] = {
      ...
      assertion match {
        ...
        case '{ (${ Expr(regex) }: Assertion.Regex).* }                                                      => Some(regex.*)
        case '{ (${ Expr(regex) }: Assertion.Regex).? }                                                      => Some(regex.?)
        case '{ (${ Expr(regex) }: Assertion.Regex).between(${ Expr(min) }, ${ Expr(max) }) }                =>
          Some(regex.between(min, max))
  ...
  given [A](using Type[A]): FromExpr[Assertion[A]] with {
    def unapply(assertion: Expr[Assertion[A]])(using Quotes): Option[Assertion[A]] = {
      ...
      assertion match {
        ...
        case '{ Assertion.between[A](${ LiteralUnlift(min) }, ${ LiteralUnlift(max) })($_) } =>
          Some(Assertion.between(min, max)(orderingForValue(min)))
        case '{ Assertion.contains(${ Expr(value) }) }                                       => Some(Assertion.contains(value).asInstanceOf[Assertion[A]])
        case '{ Assertion.divisibleBy[A](${ LiteralUnlift(value) })($_) }                    =>
          Some(Assertion.divisibleBy(value)(numericForValue(value)))
        ...
        case '{ Assertion.greaterThan[A](${ LiteralUnlift(value) })($_) }                    =>
          Some(Assertion.greaterThan(value)(orderingForValue(value)))
        ...
        case '{ Assertion.hasLength(${ Expr(assertion) }) }                                  =>
          Some(Assertion.hasLength(assertion).asInstanceOf[Assertion[A]])
        case '{ Assertion.isEmptyString }                                                    => Some(Assertion.isEmptyString.asInstanceOf[Assertion[A]])
        case '{ Assertion.lessThan[A](${ LiteralUnlift(value) })($_) }                       =>
          Some(Assertion.lessThan(value)(orderingForValue(value)))
        case '{ Assertion.lessThanOrEqualTo[A](${ LiteralUnlift(value) })($_) }              =>
          Some(Assertion.lessThanOrEqualTo(value)(orderingForValue(value)))
        case '{ Assertion.matches(${ Expr(regex) }: Assertion.Regex) }                       =>
          Some(Assertion.matches(regex).asInstanceOf[Assertion[A]])
        case '{ Assertion.matches((${ Expr(regex) }: String).r) }                            =>
          Some(Assertion.matches(regex.r).asInstanceOf[Assertion[A]])
        case '{ Assertion.matches(${ Expr(regex) }: String) }                                =>
          Some(Assertion.matches(regex).asInstanceOf[Assertion[A]])
        ...
        case '{ Assertion.powerOf[A](${ LiteralUnlift(value) })($_) }                        =>
          Some(Assertion.powerOf(value)(numericForValue(value)))
